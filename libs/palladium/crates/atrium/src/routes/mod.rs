...
/// - `POST /v1/changes`, `GET /v1/changes` (ACL-filtered, `{changes, purges, events}`)
/// - `POST /v1/changes/events/ack` (workspace-scoped event acknowledgement)
...
pub fn create_router(state: AtriumState, cors: CorsLayer) -> Router {
    Router::new()
        ...
        .route("/v1/changes/events/ack", post(changes::acknowledge_events))
        .route(
            "/v1/shares",
            post(shares::create_share).delete(shares::delete_share),
        )
        .route("/v1/records/:root_id/sharing", patch(shares::set_sharing))
        .route(
            "/v1/blobs/:blob_id",
